...
def build_match_sql(manual_hp_ids, role_position_id, min_hp_rating):
    # Convert manual HP list to SQL array
    if manual_hp_ids:
        manu = ",".join(f"'{x}'" for x in manual_hp_ids)
        manual_hp_sql = f"ARRAY[{manu}]::text[]"
    else:
        manual_hp_sql = "ARRAY[]::text[]"
...
all_numeric_scores AS (
    SELECT employee_id, pillar_code AS tv_name, score::numeric AS user_score
    FROM competencies_yearly
    JOIN latest l USING(year)
...
numeric_tv AS (
    SELECT sc.employee_id, bn.tv_name, bn.baseline_score, sc.user_score,
...
baseline_papi AS (
    ...
    LEFT JOIN reverse_list rl ON ps.scale_code = rl.scale_code
...
all_tv AS (
    ...
    UNION ALL SELECT * FROM papi_tv
    UNION ALL SELECT * FROM categorical_tv
),
...
tgv_match AS (
    SELECT a.employee_id, m.tgv_name,
...
SELECT e.employee_id, e.fullname, fm.final_match_rate
FROM final_match fm
...
def run_match_query(manual_ids, role_position_id, min_rating):
    sql = build_match_sql(manual_ids, role_position_id, min_rating)
